	...
	make
		...
		do
			...
			if attached random_word as non_void_word then
				...
				io.put_string ("%N")
				-- io.put_string (random_word) Quickly add for testing, prints word before underlines
...
feature -- Drawing Functions
	...
	display_win_popup (board: EV_DRAWING_AREA)
		local
			dialog: EV_TEXT
			colorUno: EV_COLOR
			colorTwo: EV_COLOR
		do
			create colorUno.make_with_rgb (1, 0, 0)
			create colorTwo.make_with_rgb(0, 1, 0)
			board.set_background_color(colorTwo)
			board.refresh_now
			board.clear
		end
